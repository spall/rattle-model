...
open import Functional.Forward.Exec (oracle) using (run ; run? ; maybeAll ; get ; fabricate)
...
data IdempotentCmd : (Cmd -> FileSystem -> List FileName) -> Cmd -> List (FileName × Maybe FileContent) -> FileSystem -> Set where
 IC : {f : (Cmd -> FileSystem -> List FileName)} (x : Cmd) (fs : List (FileName × Maybe FileContent)) (sys : FileSystem) -> map proj₁ fs ≡ f x sys -> CmdIdempotent x fs sys -> IdempotentCmd f x fs sys

data IdempotentState : (Cmd -> FileSystem -> List FileName) -> FileSystem -> Memory -> Set where
  [] : {f : (Cmd -> FileSystem -> List FileName)} {s : FileSystem} -> IdempotentState f s []
  Cons : {f : (Cmd -> FileSystem -> List FileName)} {x : Cmd} {fs : List (FileName × Maybe FileContent)} {s : FileSystem} {mm : Memory} -> IdempotentCmd f x fs s -> IdempotentState f s mm -> IdempotentState f s ((x , fs) ∷ mm)
...
cmdIdempotent : (sys : FileSystem) (x : Cmd) -> Disjoint (cmdReadNames x sys) (cmdWriteNames x sys) -> ∀ f₁ → St.run x (St.run x sys) f₁ ≡ St.run x sys f₁
...
cmdReadWrites : Cmd -> FileSystem -> List FileName
cmdReadWrites x sys = (cmdReadNames x sys) ++ (cmdWriteNames x sys)

lemma2 : {sys : FileSystem} {ls : Files} (fs : List (FileName × Maybe FileContent)) -> All (λ (f₁ , v₁) → foldr extend sys ls f₁ ≡ v₁) fs -> Disjoint (map proj₁ fs) (map proj₁ ls) -> All (λ (f₁ , v₁) → sys f₁ ≡ v₁) fs
lemma2 [] all₁ dsj = All.[]
...
runPreserves : {sys : FileSystem} (x x₁ : Cmd) -> (fs : List (FileName × Maybe FileContent)) -> Disjoint (cmdWriteNames x₁ sys) (cmdReadWrites x sys) -> IdempotentCmd cmdReadNames x fs sys -> IdempotentCmd cmdReadNames x fs (St.run x₁ sys)
...
preserves2 : {sys : FileSystem} {mm : Memory} (x : Cmd) -> IdempotentState cmdReadNames sys mm -> All (λ x₁ → Disjoint (cmdWriteNames x sys) (cmdReadWrites x₁ sys)) (map proj₁ mm) -> IdempotentState cmdReadNames (St.run x sys) mm
preserves2 x [] all₁ = []
preserves2 {sys} {(x₅ , _) ∷ mm} x (Cons ic is) (px All.∷ all₁) = Cons (runPreserves x₅ x _ px ic) (preserves2 x is all₁)


preserves : {sys : FileSystem} {mm : Memory} (x : Cmd) -> Disjoint (cmdReadNames x sys) (cmdWriteNames x sys) -> All (λ x₁ → Disjoint (cmdWriteNames x sys) (cmdReadWrites x₁ sys)) (map proj₁ mm) -> IdempotentState cmdReadNames sys mm -> IdempotentState cmdReadNames (St.run x sys) (save x (cmdReadNames x sys) (St.run x sys) mm)
...
getCmdIdempotent : {f : (Cmd -> FileSystem -> List FileName)} {sys : FileSystem} (mm : Memory) (x : Cmd) -> IdempotentState f sys mm -> (x∈ : x ∈ map proj₁ mm) -> CmdIdempotent x (get x mm x∈) sys
...
run≡ : (sys₁ sys₂ : FileSystem) (mm : Memory) (x : Cmd) -> (∀ f₁ → sys₁ f₁ ≡ sys₂ f₁) -> IdempotentState cmdReadNames sys₂ mm -> ∀ f₁ → St.run x sys₁ f₁ ≡ proj₁ (run (sys₂ , mm) x) f₁
