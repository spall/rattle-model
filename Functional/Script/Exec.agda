...
∃cmd-reads? : {s : System} (fs : List FileName) (b : Build) -> ∃[ x ](∃[ xs ](∃[ ys ](∃[ f₁ ](b ≡ xs ++ x ∷ ys × f₁ ∈ fs × f₁ ∈ proj₁ (trace oracle (exec s xs) x))))) ⊎ Disjoint fs (reads s b)
∃cmd-reads? fs [] = inj₂ λ ()
...
... | inj₂ dsj₂ = inj₂ λ x₁ → g₃ dsj dsj₂ (proj₁ x₁) (proj₂ x₁)
  where g₃ : {v : FileName} {ls₀ ls₁ ls₂ : Build} -> Disjoint ls₀ ls₂ -> Disjoint ls₀ ls₁ -> v ∈ ls₀ -> v ∈ ls₂ ++ ls₁ -> ⊥
        g₃ {v} {ls₀} {ls₁} {ls₂} dsj₁ dsj₂ v∈ls₀ v∈ls₂++ls₁ with ∈-++⁻ ls₂ v∈ls₂++ls₁
...
value≡ : {s s₁ : System} (f₁ : FileName) (xs : Build) -> s f₁ ≡ s₁ f₁ -> All (λ f₂ → s f₂ ≡ s₁ f₂) (reads s xs) -> exec s xs f₁ ≡ exec s₁ xs f₁
value≡ f₁ [] ≡₁ all₁ = ≡₁
value≡ {s} {s₁} f₁ (x ∷ xs) ≡₁ all₁
  = value≡ {run oracle x s} {run oracle x s₁} f₁ xs
    (St.lemma2 {oracle} {s} {s₁} x f₁ (proj₂ (oracle x) s s₁ (λ f₂ f₂∈Creads → lookup all₁ (∈-++⁺ˡ f₂∈Creads))) ≡₁)
    (St.lemma1 {oracle} {s} {s₁} (reads (run oracle x s) xs) x (++⁻ˡ (Creads s x) all₁) (++⁻ʳ (Creads s x) all₁))


lemma1 : {x₁ x₂ x₃ : Cmd} (b₁ b₂ : Build) -> x₁ before x₂ en (b₁ ++ b₂) -> x₁ before x₂ en (b₁ ++ x₃ ∷ b₂)
lemma1 {x₁} {x₂} {x₃} [] b₂ (ls₁ , ls₂ , b₂≡ls₁++x₁∷ls₂ , x₂∈ls₂)
  = (x₃ ∷ ls₁) , ls₂ , cong (x₃ ∷_) b₂≡ls₁++x₁∷ls₂ , x₂∈ls₂
lemma1 {x₁} {x₂} {x₃} (x ∷ b₁) b₂ ([] , ls₂ , b₁++b₂≡x₁∷ls₂ , x₂∈ls₂) with ∷-injective b₁++b₂≡x₁∷ls₂
... | x≡x₁ , b₁++b₂≡ls₂ = [] , b₁ ++ x₃ ∷ b₂ , cong (_∷ b₁ ++ x₃ ∷ b₂) x≡x₁ , x₂∈b₁++x₃∷b₂
  where x₂∈b₁++x₃∷b₂ : x₂ ∈ b₁ ++ x₃ ∷ b₂
        x₂∈b₁++x₃∷b₂ with ∈-++⁻ b₁ (subst (λ x₄ → x₂ ∈ x₄) (sym b₁++b₂≡ls₂) x₂∈ls₂)
        ... | inj₁ x₂∈b₁ = ∈-++⁺ˡ x₂∈b₁
        ... | inj₂ x₂∈b₂ = ∈-++⁺ʳ b₁ (there x₂∈b₂)
lemma1 {x₁} {x₂} {x₃} (x ∷ b₁) b₂ (x₄ ∷ ls₁ , ls₂ , x∷b₁++b₂≡x₄∷ls₁++x₁∷ls₂ , x₂∈ls₂) with ∷-injective x∷b₁++b₂≡x₄∷ls₁++x₁∷ls₂
... | x≡x₄ , b₁++b₂≡ls₁++x₁∷ls₂ with lemma1 b₁ b₂ (ls₁ , ls₂ , b₁++b₂≡ls₁++x₁∷ls₂ , x₂∈ls₂)
... | ls₃ , ls₄ , b₁++x₃∷b₂≡ls₃++x₁∷ls₄ , x₂∈ls₄
  = x₄ ∷ ls₃ , ls₄ , cong₂ _∷_ x≡x₄ b₁++x₃∷b₂≡ls₃++x₁∷ls₄ , x₂∈ls₄


lemma4 : {x₁ x₂ x₃ : Cmd} (b : Build) -> x₁ before x₂ en b -> x₁ before x₂ en (b ∷ʳ x₃)
lemma4 {x₁} {x₂} {x₃} b (ls₁ , ls₂ , b≡ls₁++x₁∷ls₂ , x₂∈ls₂)
  = ls₁ , ls₂ ∷ʳ x₃ , ≡₁ , (∈-++⁺ˡ x₂∈ls₂)
    where ≡₁ : b ∷ʳ x₃ ≡ ls₁ ++ x₁ ∷ ls₂ ∷ʳ x₃
          ≡₁ = subst (λ x → _ ≡ x) (CLP.l10 x₃ ls₁ (x₁ ∷ ls₂)) (cong (_∷ʳ x₃) b≡ls₁++x₁∷ls₂)
...
lemma5 : {s : System} (v w x : Cmd) (xs ys : Build) -> Disjoint (Cwrites (exec s xs) x) (reads (run oracle x (exec s xs)) ys) -> write-before-read s v w (xs ++ ys) -> write-before-read s v w (xs ++ x ∷ ys)
lemma5 {s} v w x [] ys dsj (f₁ , ls₁ , ls₂ , ls₃ , ys≡ls₁++v∷ls₂++w∷ls₃ , f₁∈writes-v , f₁∈reads-w)
  ...
  where dsj₂ : Disjoint (Cwrites s x) (reads (run oracle x s) (ls₁ ++ v ∷ ls₂ ++ w ∷ ls₃))
        ...
        all₂ = St.lemma5 {s} (reads (run oracle x s) (ls₁ ++ v ∷ ls₂ ++ w ∷ ls₃)) (proj₂ (proj₁ (oracle x) s)) dsj₂
        ...
        f₁∈reads-w₂ with h₃ {s} {run oracle x s} w (ls₁ ++ v ∷ ls₂) ls₃ (subst (λ x₁ → All (λ f₂ → s f₂ ≡ run oracle x s f₂) (reads (run oracle x s) x₁)) (CLP.l8 ls₁) all₂)
...
... | x₁≡v , xs++ys≡ls₂++w∷ls₃ with h₁ {run oracle v s} f₁ x w xs ys ls₂ ls₃ f₁∈reads-w dsj₂ xs++ys≡ls₂++w∷ls₃
  where dsj₂ : Disjoint (Cwrites (exec (run oracle v s) xs) x) (reads (run oracle x (exec (run oracle v s) xs)) ys)
...
... | x₁≡x₂ , xs++ys≡ls₁++v∷ls₂++w∷ls₃ with lemma5 {run oracle x₂ sys} v w x xs ys  dsj₂ (f₁ , ls₁ , ls₂ , ls₃ , xs++ys≡ls₁++v∷ls₂++w∷ls₃ , f₁∈writes-v , f₁∈reads-w)
  where dsj₂ : Disjoint (Cwrites (exec (run oracle x₂ sys) xs) x) (reads (run oracle x (exec (run oracle x₂ sys) xs)) ys)
...
-- not even useful..
still-wbr : {s : System} (v w x : Cmd) (xs ys : Build) -> write-before-read s v w (xs ++ ys) -> ∃[ y ](∃[ z ](write-before-read s y z (xs ++ x ∷ ys)))
...
... | inj₁ (z , ls₁ , ls₂ , f₁ , ys≡ls₁++z∷ls₂ , f₁∈writes-x , f₁∈reads-z)
  = x , z , f₁ , xs , ls₁ , ls₂
  , cong (xs ++_) (cong (x ∷_) ys≡ls₁++z∷ls₂)
  , f₁∈writes-x
  , subst (λ x₁ → f₁ ∈ proj₁ (trace oracle x₁ z)) (exec≡₁ x xs ls₁) f₁∈reads-z
... | inj₂ dsj = v , w , lemma5 {s} v w x xs ys dsj₂ wbr
  where dsj₂ : Disjoint (Cwrites (exec s xs) x) (reads (run oracle x (exec s xs)) ys)
        dsj₂ = λ x₁ → dsj (proj₁ x₁ , subst (λ x₂ → _ ∈ reads x₂ ys) (exec≡₃ {s} x xs) (proj₂ x₁))

swrh-∷ʳ⁻ : (s : System) (x : Cmd) (b b₁ b₂ : Build) -> ¬speculative-wr-hazard s (b ∷ʳ x) (b₁ ++ x ∷ b₂) -> Disjoint (Cwrites (exec s b₁) x) (reads (run oracle x (exec s b₁)) b₂) -> ¬speculative-wr-hazard s b (b₁ ++ b₂)
swrh-∷ʳ⁻ s x b b₁ b₂ ¬sp-wr-hz dsj = λ x₁ → ¬sp-wr-hz (g₁ x₁)
  where g₁ : speculative-wr-hazard s b (b₁ ++ b₂) -> speculative-wr-hazard s (b ∷ʳ x) (b₁ ++ x ∷ b₂)
