...
h₃ : {s s₁ : System} (x : Cmd) (xs ys : Build) -> All (λ f₁ → s f₁ ≡ s₁ f₁) (buildReadNames s₁ (xs ++ x ∷ ys)) -> All (λ f₁ → exec s xs f₁ ≡ exec s₁ xs f₁) (cmdReadNames x (exec s₁ xs))
...
h₁ : {s : System} (f₁ : FileName) -> (x w : Cmd) (xs ys ls₁ ls₂ : Build) -> f₁ ∈ cmdReadNames w (exec s ls₁) -> Disjoint (cmdWriteNames x (exec s xs)) (buildReadNames (run x (exec s xs)) ys) -> xs ++ ys ≡ ls₁ ++ w ∷ ls₂ -> ∃[ ls₃ ](∃[ ls₄ ](xs ++ x ∷ ys ≡ ls₃ ++ w ∷ ls₄ × f₁ ∈ cmdReadNames w (exec s ls₃)))
...
... | all₂ = (x ∷ ls₁) , ls₂ , cong (x ∷_) ys≡ls₁++w∷ls₂ , subst (λ x₁ → f₁ ∈ x₁) (cong ((map proj₁) ∘ proj₁) (h₄ (exec s ls₁) (exec (run x s) ls₁) w all₂)) f₁∈
...
... | x₁≡x₂ , xs++ys≡ls₁++w∷ls₂ with h₁ {run x₂ s} f₁ x w xs ys ls₁ ls₂ f₁∈ dsj₂ xs++ys≡ls₁++w∷ls₂
  where dsj₂ : Disjoint (cmdWriteNames x (exec (run x₂ s) xs)) (buildReadNames (run x (exec (run x₂ s) xs)) ys)
        dsj₂ = λ x₃ → dsj (subst (λ x₄ → _ ∈ cmdWriteNames x (exec (run x₄ s) xs)) (sym x₁≡x₂) (proj₁ x₃)
                          , subst (λ x₄ → _ ∈ buildReadNames (run x (exec (run x₄ s) xs)) ys) (sym x₁≡x₂) (proj₂ x₃))
