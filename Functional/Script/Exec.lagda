...
h₃ : {s s₁ : FileSystem} (x : Cmd) (xs ys : Build) -> All (λ f₁ → s f₁ ≡ s₁ f₁) (buildReadNames s₁ (xs ++ x ∷ ys)) -> All (λ f₁ → script xs s f₁ ≡ script xs s₁ f₁) (cmdReadNames x (script xs s₁))
...
h₁ : {s : FileSystem} (f₁ : FileName) -> (x w : Cmd) (xs ys ls₁ ls₂ : Build) -> f₁ ∈ cmdReadNames w (script ls₁ s) -> Disjoint (cmdWriteNames x (script xs s)) (buildReadNames (run x (script xs s)) ys) -> xs ++ ys ≡ ls₁ ++ w ∷ ls₂ -> ∃[ ls₃ ](∃[ ls₄ ](xs ++ x ∷ ys ≡ ls₃ ++ w ∷ ls₄ × f₁ ∈ cmdReadNames w (script ls₃ s)))
...
... | all₂ = (x ∷ ls₁) , ls₂ , cong (x ∷_) ys≡ls₁++w∷ls₂ , subst (λ x₁ → f₁ ∈ x₁) (cong ((map proj₁) ∘ proj₁) (h₄ (script ls₁ s) (script ls₁ (run x s)) w all₂)) f₁∈
...
... | x₁≡x₂ , xs++ys≡ls₁++w∷ls₂ with h₁ {run x₂ s} f₁ x w xs ys ls₁ ls₂ f₁∈ dsj₂ xs++ys≡ls₁++w∷ls₂
  where dsj₂ : Disjoint (cmdWriteNames x (script xs (run x₂ s))) (buildReadNames (run x (script xs (run x₂ s))) ys)
        dsj₂ = λ x₃ → dsj (subst (λ x₄ → _ ∈ cmdWriteNames x (script xs (run x₄ s))) (sym x₁≡x₂) (proj₁ x₃)
                          , subst (λ x₄ → _ ∈ buildReadNames (run x (script xs (run x₄ s))) ys) (sym x₁≡x₂) (proj₂ x₃))
