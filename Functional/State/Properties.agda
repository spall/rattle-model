...
lemma4 : {s s₁ : FileSystem} (ls : Files) -> (f₁ : FileName) -> f₁ ∈ getNames ls -> foldr extend s ls f₁ ≡ foldr extend s₁ ls f₁
...
lemma1-sym : {s s₁ : FileSystem} (ls : FileNames) -> (x : Cmd) -> All (λ f₁ → s f₁ ≡ s₁ f₁) (cmdReadNames x s₁) -> All (λ f₁ → s f₁ ≡ s₁ f₁) ls -> All (λ f₁ → run x s f₁ ≡ run x s₁ f₁) ls
...
lemma5 : {s : FileSystem} (ls : FileNames) (ls₁ : Files) -> Disjoint (getNames ls₁) ls -> All (λ f₁ → s f₁ ≡ foldr extend s ls₁ f₁) ls
lemma5 [] ls₁ dsj = All.[]
...
run-≡ : {sys sys₁ : FileSystem} (x : Cmd) -> (∀ f₁ → sys f₁ ≡ sys₁ f₁) -> (∀ f₁ → run x sys f₁ ≡ run x sys₁ f₁)
